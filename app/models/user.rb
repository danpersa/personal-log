...
class User < ActiveRecord::Base
  ...
  def self.users_with_public_or_own_reminders_for_idea(idea, user)
    ...
    where("(reminders.idea_id = :idea_id AND (reminders.privacy_id = :privacy_id OR reminders.user_id = :user_id))",
      ...
      :user_id => user).
    group("users.id, users.name, users.email, users.created_at, users.updated_at, users.activation_code, users.encrypted_password, users.password_reset_code, users.activated_at, users.reset_password_mail_sent_at, users.state, users.admin, users.salt, reminders.created_at").
